...
0. 자료가 있는가? (발표자료, 스크린샷, 참고 사이트 URL 등)
   → **발표자료 전체(PDF로 내보낸 덱, 또는 슬라이드 이미지 폴더)가 있으면 `deck-to-video`
     스킬로 가라** — 슬라이드 인제스트부터 SCRIPT.md 승인까지 이 스킬 대신 그쪽이 담당한다.
   → 그 외 자료(낱장 스크린샷, 사진, 다이어그램 몇 장)면 아래 "자료를 받았을 때" 절을 따라
     하나씩 짚으며 제안·확인한 뒤 /hyperframes 를 불러라. 넘겨짚고 한꺼번에 처리하지 않는다.
