...
for (const line of script) {
  ...
  // ── 규칙 7: 나레이션 속 라틴문자 ──
  // Piper 한국어 음성은 라틴문자를 읽지 못한다. 대본에 남아 있으면 소리가 깨지거나,
  // 살아남으려고 음차("re0" → "리제로")해서 화면 표기와 어긋난다.
  // 읽지 않는 URL 등 정당한 예외가 있으므로 경고만 한다 — 화면과 일치하는지는 눈으로 봐야 한다.
  for (const m of line.text.matchAll(/[A-Za-z][A-Za-z0-9._-]*/g)) {
    warn(7, at,
      `나레이션에 라틴문자 "${m[0]}" — Piper 가 읽지 못합니다. 한국어 용어로 바꾸거나, ` +
      `화면에만 띄우고 나레이션에서는 빼세요 (references/korean-terminology.md)`);
  }
